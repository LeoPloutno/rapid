...
use crate::potential::physical::MonteCarloAtomDecoupledPhysicalPotential;
...
pub trait MonteCarloGroupDecoupledPhysicalPotential<T, V>: GroupDecoupledPhysicalPotential<T, V> {
    ...
    #[must_use = "Discarding the result of a potentially heavy computation is wasteful"]
    fn calculate_potential_diff(&mut self, changed_position_idx: usize, old_value: V, group_positions: &[V]) -> T;
