...
pub trait GroupDecoupledPhysicalPotential<T, V> {
    ...
    fn calculate_potential_set_forces(&mut self, group_positions: &[V], group_forces: &mut [V]) -> T;
    ...
    /// Returns the contribution to the total energy.
    #[must_use = "Discarding the result of a potentially heavy computation is wasteful"]
    fn calculate_potential_add_forces(&mut self, group_positions: &[V], group_forces: &mut [V]) -> T;
    ...
    #[must_use = "Discarding the result of a potentially heavy computation is wasteful"]
    fn calculate_potential(&mut self, group_positions: &[V]) -> T;

    /// Sets the group_forces of this group.
    #[deprecated = "Consider using `calculate_potential_set_forces` as a more efficient alternative"]
    fn set_forces(&mut self, group_positions: &[V], group_forces: &mut [V]);
    ...
    fn add_forces(&mut self, group_positions: &[V], group_forces: &mut [V]);
}
    ...
    U: AtomDecoupledPhysicalPotential<T, V> + ?Sized,
{
        ...
        let mut iter =
            group_positions
                ...
                .map(|(idx, (position, force))| {
                    AtomDecoupledPhysicalPotential::calculate_potential_set_force(self, idx, position, force)
                });
    ...
    fn calculate_potential_add_forces(&mut self, group_positions: &[V], group_forces: &mut [V]) -> T {
        let mut iter =
            group_positions
                ...
                .enumerate()
                .map(|(idx, (position, force))| {
                    AtomDecoupledPhysicalPotential::calculate_potential_add_force(self, idx, position, force)
                });
        let first_atom_energy = iter.next().expect("There must be at least one atom in a group");
        iter.fold(first_atom_energy, |accum_energy, atom_energy| {
            accum_energy + atom_energy
    ...
    fn calculate_potential(&mut self, group_positions: &[V]) -> T {
        let mut iter = group_positions.iter().enumerate().map(|(idx, position)| {
            #[allow(deprecated)]
            AtomDecoupledPhysicalPotential::calculate_potential(self, idx, position)
        });
        let first_atom_energy = iter.next().expect("There must be at least one atom in a group");
        iter.fold(first_atom_energy, |accum_energy, atom_energy| {
            accum_energy + atom_energy
    ...
    fn set_forces(&mut self, group_positions: &[V], group_forces: &mut [V]) {
        for (idx, (position, force)) in group_positions.iter().zip(group_forces.iter_mut()).enumerate() {
            #[allow(deprecated)]
            AtomDecoupledPhysicalPotential::set_force(self, idx, position, force);
        }
    }
