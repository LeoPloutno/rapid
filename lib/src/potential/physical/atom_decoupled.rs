...
pub trait AtomDecoupledPhysicalPotential<T, V> {
    ...
    fn calculate_potential_set_force(&mut self, atom_idx: usize, position: &V, force: &mut V) -> T;
    ...
    /// Returns the contribution to the total energy.
    #[must_use = "Discarding the result of a potentially heavy computation is wasteful"]
    fn calculate_potential_add_force(&mut self, atom_idx: usize, position: &V, force: &mut V) -> T;
    ...
    #[must_use = "Discarding the result of a potentially heavy computation is wasteful"]
    fn calculate_potential(&mut self, atom_idx: usize, position: &V) -> T;

    /// Sets the force of this atom.
    #[deprecated = "Consider using `calculate_potential_set_force` as a more efficient alternative"]
    fn set_force(&mut self, atom_idx: usize, position: &V, force: &mut V);
    ...
    fn add_force(&mut self, atom_idx: usize, position: &V, force: &mut V);
}
